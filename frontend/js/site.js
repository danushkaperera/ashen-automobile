...
function initStaticForms() {
    const workshopEmail = 'service@autobridge.co.nz';
    document.querySelectorAll('form[data-mailto]').forEach((form) => {
        form.addEventListener('submit', (event) => {
            ...
            window.location.href = `mailto:${workshopEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(lines)}`;
            form.querySelectorAll('.flash-success').forEach((node) => node.remove());
            const flash = document.createElement('div');
            flash.className = 'flash-success';
            flash.textContent = 'Your email app should open with the message. If it does not, call +64 29 020 16792.';
            form.prepend(flash);
        });
    });
}
